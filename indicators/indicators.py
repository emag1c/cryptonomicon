...
def concat_frames(a: pd.DataFrame, b: pd.DataFrame, **kwargs):
    concat_kwargs = {
        "axis": 1,
        "ignore_index": False,
        "sort": False,
    ...
    return pd.concat((a, b), **concat_kwargs)
...
class Indicator:
    ...
    def concat(self, frame: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
        return concat_frames(frame, self.to_frame(*args, **kwargs), **kwargs)
...
class Set:
    ...
    def to_frame(self, *args, **kwargs) -> pd.DataFrame:
        ...
        for i in self.indicators:
            if df is None:
                df = i.to_frame(*args, **kwargs)
            else:
                df = pd.concat((df, i.to_frame(*args, **kwargs)), **concat_kwargs)
        return df

    def concat(self, frame: pd.DataFrame, *args, **kwargs):
        return concat_frames(frame, self.to_frame(*args, **kwargs), **kwargs)

    def _exec(self, *args, **kwargs):
        outputs = []
        for i in self.indicators:
            outputs.append(i(*args, **kwargs))
        return outputs
...
class ATR(Indicator):
    ...
    def __init__(self, period: int=14, name=None):
        assert period > 0
        params = [
            Parameter("p", period),
        ]
...
class SMASet(Indicator):
    """
    set of simple moving averages
    """

    def __init__(self, periods: Union[Tuple[int], List[int]], name="SMA"):
        assert len(periods) > 0
        self.periods = periods
        outputs = []
        for p in periods:
            outputs.append(str(p))
        super(SMASet, self).__init__(None, outputs, name=name)

    def _exec(self, series: Union[np.array, pd.Series]) -> Tuple[np.array]:
        outputs: List[np.array] = []
        for p in self.periods:
            outputs.append(np.array(ema(series, p)))
...
class EMASet(Indicator):
    """
    set of exponential moving averages
    """

    def __init__(self, periods: Union[Tuple[int], List[int]], name="EMA"):
        assert len(periods) > 0
        self.periods = periods
        outputs = []
        for p in periods:
            outputs.append(str(p))
        super(EMASet, self).__init__(None, outputs, name=name)
...
def lin_v(a: np.array) -> np.float:
    """
    get last linear regression value of input array

    :param a: input numpy array
    :return: the linear regression value
    """
    x = np.array(range(0, len(a))).reshape(-1, 1)
    y = a.reshape(-1, 1)
    lr = LinearRegression()
...
def lin(a: np.array) -> pd.DataFrame:
    """
    linear regression
    """
    x = np.array(range(0, len(a))).reshape(-1, 1)
...
class KelBbandDif(Indicator):
    ...
    def __init__(self, period=20, bband_dev=2., kel_multiplier=2, name=None):
        ...
        super(KelBbandDif, self).__init__(params, ["Upper", "Lower"], name=name)
...
class CandlePatterns(Indicator):
    ...
    def __init__(self, norm=False, output=None, name=None):
        self.norm = norm
        if output is not None:
            for o in output:
                assert o in DEFAULT_CANDLE_PATTERNS
        else:
            output = DEFAULT_CANDLE_PATTERNS

        super(CandlePatterns, self).__init__(outputs=output, name=name)

    def normalize(self, a: np.array) -> np.array:
        """
        convert -100, 0, 100 outputs to 0, .5, 1 for use with ML
        :param a: input array
        :return: output array
        """
        a = np.where(a > 1, 1, a)
